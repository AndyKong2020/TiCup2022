...
def land():
    ...
    if not(roi[0] > sensor.width() -1 or roi[0] < 1- set_range[1] or roi[1] > sensor.height() - 1 or roi[1] < 1 - set_range[0]) :
        ...
        #img.erode(1)
        img.dilate(2)
        found_blobs = img.find_blobs([(10, 255)], roi=roi, x_stride = 30, y_stride = 30, pixels_threshold = 1000)
        max_blob = find_maxblob(found_blobs)
        found_blobs_merge = img.find_blobs([(10, 255)], roi=roi, x_stride = 30, y_stride = 30, pixels_threshold = 100, merge = True, margin = 100)
        found_blobs_merge = find_maxblob(found_blobs_merge)

        if max_blob:
            if found_blobs_merge.pixels() > 0.5*set_range[0]*set_range[1] or found_blobs_merge.area() > 1*set_range[0]*set_range[1]:
                if (accurate_roi_inner[0] < sensor.height()) and (accurate_roi_inner[1] > 0) and (accurate_roi_inner[2] > 0) and (accurate_roi_inner[3] < sensor.width()):
                    found_top_points = img.find_blobs([(0, 1)], roi = accurate_roi_top, x_stride = 15, pixels_threshold = 40)
                    max_top_point = find_maxblob(found_top_points)
                    found_bottom_points = img.find_blobs([(0, 1)], roi = accurate_roi_bottom, x_stride = 15, pixels_threshold = 40)
                    max_bottom_point = find_maxblob(found_bottom_points)
                    found_left_points = img.find_blobs([(0, 1)], roi = accurate_roi_left, y_stride = 15, pixels_threshold = 40)
                    max_left_point = find_maxblob(found_left_points)
                    found_right_points = img.find_blobs([(0, 1)], roi = accurate_roi_right, y_stride = 15, pixels_threshold = 40)
