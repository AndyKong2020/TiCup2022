...
u8 Height_increase(u16 target_height,u16 velocity_cms)
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[1] = 0X01;
		//
		dt.cmd_send.CMD[2] = BYTE0(target_height);
		dt.cmd_send.CMD[3] = BYTE1(target_height);
...
u8 Height_decrease(u16 target_height,u16 velocity_cms)
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
