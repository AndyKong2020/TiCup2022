...
_dt_st dt;
s32 height_mix,height_laser;

s16 imu_speed_x,imu_speed_y,imu_speed_z;
...
void ANO_DT_Init(void)
{
	...
	dt.fun[0xf1].fre_ms = 20;	  //触发发送的周期100ms
...
static void ANO_DT_LX_Data_Receive_Anl(u8 *data, u8 len)
{
	...
	{
		height_laser=*((s16 *)(data+8));//激光高度
		height_mix=*((s16 *)(data+4));//融合高度
	}
	//凌霄飞控当前的运行状态
	else if (*(data + 2) == 0X06)
	{
	...
	else if (*(data + 2) == 0X07)
	{
		...
		imu_speed_x = *((s16 *)(data + 4));
		imu_speed_y = *((s16 *)(data + 6));
		imu_speed_z = *((s16 *)(data + 8));
	}
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
	...
	case 0xf1: //test
	{
		send_buffer[(*_cnt)++] =	color_identify_data[0]%256;
		send_buffer[(*_cnt)++] =	color_identify_data[0]/256;
		
		send_buffer[(*_cnt)++] = 	color_identify_data[1]%256;
		send_buffer[(*_cnt)++] = 	color_identify_data[1]/256;
		
		send_buffer[(*_cnt)++] = BYTE0(x_range);
		send_buffer[(*_cnt)++] = BYTE1(x_range);
		
		send_buffer[(*_cnt)++] = BYTE0(y_range);
		send_buffer[(*_cnt)++] = BYTE1(y_range);
		
		
		send_buffer[(*_cnt)++] = BYTE0(i);
		send_buffer[(*_cnt)++] = BYTE1(i);
		
		send_buffer[(*_cnt)++] = BYTE0(height_laser);
		send_buffer[(*_cnt)++] = BYTE1(height_laser);
		send_buffer[(*_cnt)++] = BYTE0(height_mix);
		send_buffer[(*_cnt)++] = BYTE1(height_mix);
		
		send_buffer[(*_cnt)++] = mission_step;
//		
//		send_buffer[(*_cnt)++] = BYTE0(b);
//		send_buffer[(*_cnt)++] = BYTE1(b);
		
//		send_buffer[(*_cnt)++] = BYTE0(QR_data[0]);
//		send_buffer[(*_cnt)++] = BYTE1(QR_data[0]);
//		send_buffer[(*_cnt)++] = BYTE0(QR_data[1]);
//		send_buffer[(*_cnt)++] = BYTE1(QR_data[1]);
//		send_buffer[(*_cnt)++] = BYTE0(QR_data[2]);
//		send_buffer[(*_cnt)++] = BYTE1(QR_data[2]);
//		
