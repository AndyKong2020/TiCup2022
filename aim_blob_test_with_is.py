...
black_thresholds = [(0, 40, -53, 20, -20, 55)]
red_blue_thresholds = [(9, 59, 15, 127, -7, 72), (1, 43, -22, 12, -128, 8)]
...
set_accurate_roi_range = [5, 0.65]#设置精准降落的roi，第一个值是宽度像素数量，第二个是长度占全roi的比值
...
sensor.set_auto_exposure(30000)
sensor.set_auto_whitebal(False, (62, 58, 64))
...
while True:
    if pin1.value():
        ...
        if not(roi[0] > sensor.width() -1 or roi[0] < 1- set_range[0] or roi[1] > sensor.height() - 1 or roi[1] < 1 - set_range[1]) :
            ...
            img.binary(black_thresholds)
            #img.erode(1)
            img.dilate(2)
            found_blobs = img.find_blobs([(10, 255)], roi=roi, x_stride = 30, y_stride = 30, pixels_threshold = 300, merge = True, margin = 50)
            max_blob = find_maxblob(found_blobs)
            found_blobs_merge = img.find_blobs([(10, 255)], roi=roi, x_stride = 15, y_stride = 15, pixels_threshold = 50, merge = True, margin = 100)
            found_blobs_merge = find_maxblob(found_blobs_merge)

            if max_blob:
                if found_blobs_merge.pixels() > 0.6*set_range[0]*set_range[1] or found_blobs_merge.area() > 0.9*set_range[0]*set_range[1]:
                    if (accurate_roi_inner[0] < sensor.height()) and (accurate_roi_inner[1] > 0) and (accurate_roi_inner[2] > 0) and (accurate_roi_inner[3] < sensor.width()):
                        found_top_points = img.find_blobs([(10, 255)], roi = accurate_roi_top, x_stride = 15, pixels_threshold = 40, invert = True)
                        max_top_point = find_maxblob(found_top_points)
                        found_bottom_points = img.find_blobs([(10, 255)], roi = accurate_roi_bottom, x_stride = 15, pixels_threshold = 40, invert = True)
                        max_bottom_point = find_maxblob(found_bottom_points)
                        found_left_points = img.find_blobs([(10, 255)], roi = accurate_roi_left, y_stride = 15, pixels_threshold = 40, invert = True)
                        max_left_point = find_maxblob(found_left_points)
                        found_right_points = img.find_blobs([(10, 255)], roi = accurate_roi_right, y_stride = 15, pixels_threshold = 40, invert = True)
        ...
        if not(roi[0] > sensor.width() -1 or roi[0] < 1- set_range[0] or roi[1] > sensor.height() - 1 or roi[1] < 1 - set_range[1] ):
            ...
            found_blobs = img.find_blobs(red_blue_thresholds, roi=roi, x_stride = 10, y_stride = 10, pixels_threshold = 100)
            max_blob = find_maxblob(found_blobs)
